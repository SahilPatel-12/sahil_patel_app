...
const MOCK_POOJAS = [
  ...
  },
  {
    ...
    pandit: 'Acharya Vishwanath Dwivedi - Senior Priest',
...
// --- Main Component --- //
const GeneralPoojasManagerPage = () => {
  const [poojas, setPoojas] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [successMsg, setSuccessMsg] = useState(null);
  
  const [previewMode, setPreviewMode] = useState('puja'); // 'puja', 'store', 'detail'
  ...
  const fetchPoojas = async () => {
    ...
    try {
      ...
      console.error('Error fetching general poojas:', err);
  ...
  const handleImageUpload = async (e, type = 'main') => {
    ...
    try {
      const publicUrl = await uploadToR2(file, 'general-poojas');
      if (type === 'main') {
      ...
      showMessage('Image uploaded to Cloudflare successfully!');
    } catch (err) {
  ...
  const handleEdit = (pooja) => {
    setFormData({
      ...
      single_price: pooja.single_price || '₹501',
      single_original_price: pooja.single_original_price || '₹1,501',
      ...
      family_price: pooja.family_price || '₹1,001',
      family_original_price: pooja.family_original_price || '₹3,001',
  ...
  const getMergedSimulatorData = (targetView) => {
    ...
    if (!isEditingFound && (formData.title || formData.provider || formData.image_url)) {
      items.unshift({ id: 'temp-preview-id', ...formData });
    }

    if (targetView === 'puja') {
      items = items.filter(p => p.id === isEditingId || p.id === 'temp-preview-id' || (p.is_active && p.status === 'published'));
      items.sort((a, b) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (isUuid) {
        const { error: delErr } = await supabase.from('general_poojas').delete().eq('id', id);
  ...
  return (
    <div className="page-container" style={{ color: '#f8fafc', padding: '24px' }}>
      ...
      <div className="page-header" style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 className="page-title" style={{ fontSize: '28px', fontWeight: '800', margin: 0 }}>Vedic Pujas Manager</h1>
      ...
      <div className="manager-split-layout">
        ...
        <div className="manager-form-section" style={{ flex: 1 }}>
          <div className="glass-card" style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '24px', backgroundColor: '#161622', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.1)', marginBottom: '24px', gap: '8px', overflowX: 'auto' }}>
              <button type="button" onClick={() => setFormTab('basic')} style={{ padding: '10px 16px', borderBottom: formTab === 'basic' ? '3px solid #ea580c' : '3px solid transparent', color: formTab === 'basic' ? '#ffffff' : '#94a3b8', fontWeight: '700', fontSize: '14px', whiteSpace: 'nowrap' }}>1. Basic Info</button>
              <button type="button" onClick={() => setFormTab('detail')} style={{ padding: '10px 16px', borderBottom: formTab === 'detail' ? '3px solid #ea580c' : '3px solid transparent', color: formTab === 'detail' ? '#ffffff' : '#94a3b8', fontWeight: '700', fontSize: '14px', whiteSpace: 'nowrap' }}>2. Puja Details</button>
              <button type="button" onClick={() => setFormTab('packages')} style={{ padding: '10px 16px', borderBottom: formTab === 'packages' ? '3px solid #ea580c' : '3px solid transparent', color: formTab === 'packages' ? '#ffffff' : '#94a3b8', fontWeight: '700', fontSize: '14px', whiteSpace: 'nowrap' }}>3. Packages</button>
              <button type="button" onClick={() => setFormTab('combo')} style={{ padding: '10px 16px', borderBottom: formTab === 'combo' ? '3px solid #ea580c' : '3px solid transparent', color: formTab === 'combo' ? '#ffffff' : '#94a3b8', fontWeight: '700', fontSize: '14px', whiteSpace: 'nowrap' }}>4. Combo Offer</button>
              <button type="button" onClick={() => setFormTab('faqs')} style={{ padding: '10px 16px', borderBottom: formTab === 'faqs' ? '3px solid #ea580c' : '3px solid transparent', color: formTab === 'faqs' ? '#ffffff' : '#94a3b8', fontWeight: '700', fontSize: '14px', whiteSpace: 'nowrap' }}>5. Divine FAQs</button>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', paddingRight: '12px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    <div className="form-group">
                      <label>Title *</label>
                      <input type="text" name="title" value={formData.title} onChange={handleInputChange} className="input-field" placeholder="e.g. Ganesh Puja" />
                    </div>
                    <div className="form-group">
                      <label>Sacred Provider (Temple) *</label>
                      <input type="text" name="provider" value={formData.provider} onChange={handleInputChange} className="input-field" placeholder="e.g. Siddhi Vinayak Mandir" />
                    </div>
                    <div className="form-group">
                    ...
                    <div style={{ display: 'flex', gap: '15px', alignItems: 'center', marginTop: '4px' }}>
                      ...
                      {formData.image_url ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          ...
                          <span style={{ fontSize: '12px', color: '#10b981' }}>Image Uploaded!</span>
                        </div>
                      ) : (
                  ...
                  {/* Display Sorting Numbering */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px', marginTop: '8px' }}>
                    <div className="form-group">
                      <label>Home Page Numbering</label>
                      <input type="number" name="sort_order_home" value={formData.sort_order_home} onChange={handleInputChange} className="input-field" placeholder="e.g. 1, 2..." />
                    </div>
                    <div className="form-group">
                      <label>Puja Tab Numbering</label>
                      <input type="number" name="sort_order_puja" value={formData.sort_order_puja} onChange={handleInputChange} className="input-field" placeholder="e.g. 1, 2..." />
                    </div>
                    <div className="form-group">
                      <label>Store Page Numbering</label>
                      <input type="number" name="sort_order_store" value={formData.sort_order_store} onChange={handleInputChange} className="input-field" placeholder="e.g. 1, 2..." />
                    </div>
                  </div>
              ...
              {formTab === 'detail' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <div className="form-group">
                    <label>Other Info / Disclaimer</label>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                        <div className="form-group">
                          ...
                          <input type="text" name="single_price" value={formData.single_price} onChange={handleInputChange} className="input-field" />
                        </div>
                        <div className="form-group">
                          <label>Single Original Strike Price</label>
                          <input type="text" name="single_original_price" value={formData.single_original_price} onChange={handleInputChange} className="input-field" />
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                        <div className="form-group">
                          ...
                          <input type="text" name="family_price" value={formData.family_price} onChange={handleInputChange} className="input-field" />
                        </div>
                        <div className="form-group">
                          <label>Family Original Strike Price</label>
                          <input type="text" name="family_original_price" value={formData.family_original_price} onChange={handleInputChange} className="input-field" />
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              {formTab === 'combo' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <div className="form-group">
                    ...
                    <textarea name="description" value={formData.combo_offer?.description || ''} onChange={handleComboChange} className="input-field" rows="3" />
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    <div className="form-group">
                      <label>Combo Original Price</label>
                      <input type="text" name="original_price" value={formData.combo_offer?.original_price || ''} onChange={handleComboChange} className="input-field" />
                    </div>
                    <div className="form-group">
                      <label>Combo Offer Price</label>
                      <input type="text" name="offer_price" value={formData.combo_offer?.offer_price || ''} onChange={handleComboChange} className="input-field" />
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    {formData.faqs.map((faq, index) => (
                      <div key={index} style={{ background: 'rgba(255,255,255,0.03)', padding: '16px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)', position: 'relative' }}>
                        <button type="button" onClick={() => removeFaq(index)} style={{ position: 'absolute', top: '12px', right: '12px', color: '#ef4444', cursor: 'pointer' }}>
                          <X size={16} />
                        </button>
                        <div className="form-group" style={{ marginBottom: '10px' }}>
                          <label style={{ fontSize: '12px', color: '#cbd5e1' }}>Question {index + 1}</label>
                          <input type="text" value={faq.q} onChange={(e) => handleFaqChange(index, 'q', e.target.value)} className="input-field" />
                        </div>
                        <div className="form-group" style={{ marginBottom: 0 }}>
                          <label style={{ fontSize: '12px', color: '#cbd5e1' }}>Answer</label>
                          <textarea value={faq.a} onChange={(e) => handleFaqChange(index, 'a', e.target.value)} className="input-field" rows="2" />
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="manager-preview-section" style={{ width: '400px', flexShrink: 0 }}>
          ...
          <div className="preview-toggle" style={{ display: 'flex', width: '100%', marginBottom: '16px', gap: '4px' }}>
            <button className={previewMode === 'puja' ? 'active' : ''} onClick={() => setPreviewMode('puja')} style={{ flex: 1, padding: '8px 4px', fontSize: '12px', fontWeight: 'bold' }}>Puja Page</button>
            <button className={previewMode === 'store' ? 'active' : ''} onClick={() => setPreviewMode('store')} style={{ flex: 1, padding: '8px 4px', fontSize: '12px', fontWeight: 'bold' }}>View Page</button>
            <button className={previewMode === 'detail' ? 'active' : ''} onClick={() => setPreviewMode('detail')} style={{ flex: 1, padding: '8px 4px', fontSize: '12px', fontWeight: 'bold' }}>Details Page</button>
          </div>
          ...
          <div className="phone-simulator" style={{ width: '320px', height: '650px', background: '#f8fafc', borderRadius: '40px', border: '8px solid #1e293b', position: 'relative', overflow: 'hidden', boxShadow: '0 25px 50px rgba(0,0,0,0.5)' }}>
            ...
            <div className="phone-content" style={{ height: '100%', position: 'relative', backgroundColor: (previewMode === 'puja' || previewMode === 'store') ? '#ffffff' : '#f8fafc', overflow: 'hidden' }}>
              ...
              {previewMode === 'store' && (
                <div style={{ display: 'flex', flexDirection: 'column', backgroundColor: '#ffffff', height: '100%', overflowY: 'auto', paddingTop: '22px', fontFamily: '"Outfit", -apple-system, sans-serif', position: 'relative' }}>
                  ...
                  <div style={{ width: '100%', height: '120px', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
                    <img src="https://pub-3027d8d3defe4496978413d3c630aa44.r2.dev/Omkarashwar.png" alt="Store Banner" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  ...
                  {/* Store Title Section */}
                  <div style={{ padding: '12px 16px 4px 16px', textAlign: 'left', flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <div style={{ backgroundColor: '#ea580c', color: 'white', borderRadius: '4px', padding: '1px 6px', fontWeight: '800', fontSize: '13px' }}>ALL</div>
                      <span style={{ fontSize: '15px', fontWeight: '800', color: '#000000', textTransform: 'lowercase' }}>pujas</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px' }}>
                      <div style={{ width: '12px', height: '12px', borderRadius: '6.5px', border: '1.5px solid black', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '8px', fontWeight: '900' }}>✓</div>
                  ...
                  {/* Horizontal Scroll Category Tab Row */}
                  <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingHorizontal: '16px', marginVertical: '10px', flexShrink: 0 }}>
                    <span style={{ backgroundColor: '#ea580c', color: 'white', padding: '4px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '700' }}>All</span>
                    <span style={{ backgroundColor: '#f1f5f9', color: '#475569', padding: '4px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '500', border: '0.5px solid #cbd5e1' }}>Wealth</span>
                    <span style={{ backgroundColor: '#f1f5f9', color: '#475569', padding: '4px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '500', border: '0.5px solid #cbd5e1' }}>Mahadev</span>
                    <span style={{ backgroundColor: '#f1f5f9', color: '#475569', padding: '4px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '500', border: '0.5px solid #cbd5e1' }}>Protection</span>
                  </div>

                  {/* Stunning 3-Column Grid */}
                  <div style={{ 
                    display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px 8px', 
                    paddingHorizontal: '12px', paddingBottom: '90px', overflowY: 'auto'
                  }}>
                  ...
                  {/* Floating Teal Free Delivery Footer */}
                  <div style={{ 
                    position: 'absolute', bottom: '16px', left: '16px', right: '16px',
                    backgroundColor: '#f0fdfa', border: '1px solid #ccfbf1', borderRadius: '20px',
              ...
              {/* --- PREVIEW MODE: PUJA TAB (VERTICAL SCROLL VIEW LIST) --- */}
              {previewMode === 'puja' && (
                <div style={{ display: 'flex', flexDirection: 'column', backgroundColor: '#ffffff', height: '100%', overflow: 'hidden', paddingTop: '22px', fontFamily: '"Outfit", -apple-system, sans-serif' }}>
                  {/* Mock App Header */}
                  ...
                  {/* Header Title */}
                  <div style={{ padding: '12px 16px 4px 16px', textAlign: 'left', flexShrink: 0 }}>
                    <h3 style={{ margin: 0, fontSize: '14px', fontWeight: '900', color: '#000000', letterSpacing: '0.5px' }}>ALL PUJA</h3>
                  </div>

                  {/* Scrollable vertical list of cards */}
                  <div style={{ flex: 1, overflowY: 'auto', padding: '0 16px 80px 16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    {getMergedSimulatorData('puja').map((item) => {
                      const displayImg = item.image_url || DEFAULT_IMAGE;
                      return (
                        <div 
                          key={item.id}
                          style={{
                        ...
                        >
                          ...
                          <div style={{ position: 'relative', width: '80px', height: '80px', borderRadius: '12px', overflow: 'hidden', flexShrink: 0, backgroundColor: '#e2e8f0' }}>
                            <img src={displayImg} alt="Puja" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                            <div style={{
                              position: 'absolute', bottom: '4px', left: '4px',
                              backgroundColor: '#10b981', color: 'white', padding: '1.5px 4px', borderRadius: '4px',
                              fontSize: '8px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '2px'
                            }}>
                              ★ {item.rating || '4.8'}
                            </div>
                          </div>
                          ...
                          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                              {/* Sparkles VEDIC SEVA Badge */}
                              <div style={{ display: 'flex', alignItems: 'center', gap: '3px', backgroundColor: '#fff5eb', padding: '2px 6px', borderRadius: '4px', border: '0.5px solid #fed7aa' }}>
                                <span style={{ color: '#ea580c', fontSize: '7.5px', fontWeight: '800' }}>✨ VEDIC SEVA</span>
                              </div>
                              <span style={{ fontSize: '8.5px', color: '#64748b', fontWeight: '500' }}>
                                {item.requirement || '45-60 mins'}
                              </span>
                            </div>

                            <h4 style={{ margin: '4px 0 2px 0', fontSize: '12px', fontWeight: '800', color: '#0f172a' }}>
                              {item.title || 'Ganesh Puja'}
                            </h4>
                            <p style={{ margin: '0 0 4px 0', fontSize: '9px', color: '#64748b', fontWeight: '500' }}>
                              by {item.provider || 'Siddhi Vinayak Mandir'}
                            </p>

                            {/* Prasad shipping text with gift box icon */}
                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#ea580c', marginBottom: '6px' }}>
                              <span style={{ fontSize: '9px' }}>🎁</span>
                              <span style={{ fontSize: '8.5px', fontWeight: '700' }}>Includes Sacred Prasad box</span>
                            </div>

                            {/* Bottom row: Price + Book Seva button */}
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                              <span style={{ fontSize: '13px', fontWeight: '800', color: '#000000' }}>
                                {item.offer_price || '₹501'}
                              </span>

                              <div style={{
                                backgroundColor: '#059669', color: 'white', padding: '4px 10px', borderRadius: '8px',
                                fontSize: '9.5px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '2px'
                              }}>
                                Book Seva ➔
                              </div>
                            </div>
      ...
      <div className="glass-card" style={{ marginTop: '24px', backgroundColor: '#161622', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.08)', padding: '24px' }}>
        <div className="card-header" style={{ marginBottom: '16px' }}>
          <h2 className="card-title" style={{ fontSize: '18px', fontWeight: '700', margin: 0 }}>All Active Vedic Pujas</h2>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '24px', color: '#94a3b8', fontStyle: 'italic' }}>
            No records found. Complete the form and click "Publish Now" to add your first Puja.
          </div>
        ) : (
...
export default GeneralPoojasManagerPage;
