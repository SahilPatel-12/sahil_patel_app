...
import { useLanguage } from '../context/LanguageContext';import { supabase } from '../services/supabase';

const { width } = Dimensions.get('window');
// Subtract a sub-pixel safety margin (1.5px) to prevent layout engine round-offs from forcing the 3rd card to wrap
const CARD_WIDTH = Math.floor((width - 32 - 20) / 3) - 1.5;

export default function OneRupeeStoreScreen() {
  const { t } = useLanguage();
  ...
  React.useEffect(() => {
    async function loadPoojas() {
      try {
        const { data, error } = await supabase
          ...
          .eq('status', 'published')
          .eq('is_active', true)
          .order('sort_order_store', { ascending: true, nullsFirst: false })
        ...
        if (data) {
          ...
          setPoojas(formatted);
          const uniqueCats = ['All', ...new Set(formatted.map(item => item.category))] as string[];
          setCategories(uniqueCats);
        }
      } catch (err) {
        console.error('Error fetching one rupee store items:', err);
      } finally {
        setIsLoading(false);
      }
    }
    ...
    const subscription = supabase
      .channel('store_one_rupee_poojas')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'one_rupee_poojas' }, (payload) => {
        console.log('[Store One Rupee] Realtime update received!', payload);
        loadPoojas();
      })
      .subscribe();
  ...
  const filteredItems = selectedCategory === 'All' 
    ? poojas 
    : poojas.filter(item => item.category === selectedCategory);

  const row1Items = filteredItems.filter((_, index) => index % 2 === 0);
  const row2Items = filteredItems.filter((_, index) => index % 2 !== 0);
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <View style={styles.headerContainer}>
        ...
        <Image
          source={require('../assets/banner/ChatGPT Image May 26, 2026, 02_08_19 AM.png')}
      ...
      >
        ...
        <View style={styles.sectionHeader}>
          <View style={styles.storeLogoContainer}>
            ...
            <Text style={styles.storeLogoTitle}>{t('store')}</Text>
          </View>
          <View style={styles.storeSubHeader}>
            <Ionicons name="checkmark-circle" size={13} color="#checkmark-circle" style={styles.checkmarkIcon} />
        ...
        {isLoading ? (
          <View style={{ padding: 20, alignItems: 'center' }}><Text>Loading...</Text></View>
        ) : filteredItems.length === 0 ? (
          <View style={{ padding: 20, alignItems: 'center' }}><Text>No Poojas found.</Text></View>
        ) : (
